...
3. The test script that tests for successful environment setup.
...
10. Maximize the number of successfully executed commands in the test script.
...
11. You may run commands from the testing script to ensure the repository is functional.
12. If verification fails, continue troubleshooting and repeat setup steps until it passes.

**Final Completion Signal:**
13. When setup is complete, output exactly
```
...
class EasyTestAgent():
    def __init__(self, test_number: int):
        ...
        with open(f"{test_number}.txt", "r") as test_file:
